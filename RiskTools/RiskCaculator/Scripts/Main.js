    ...
    while (Attacker > 1 && Defender > 0 && Attacker > GiveUp) {
        ...
        for (let i = 0; i < Math.min(attackerDice.length, defenderDice.length); i++) {
            if (attackerDice[i] > defenderDice[i]) {
                Defender--; // Decrease defender troops
                if (Defender < 0) {
                    Defender = 0; // Prevent defender from going below 0
                }
            } else {
                Attacker--; // Decrease attacker troops
                if (Attacker < 1) {
                    Attacker = 1; // Prevent attacker from going below 1
                }
            }
        }
        ...
        // If defender is out of troops, break the loop
        if (Defender <= 0) {
            break;
        }
    }
